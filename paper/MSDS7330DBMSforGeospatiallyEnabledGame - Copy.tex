...
Player Registration: Record user profile data (UserID, name, email address) from the client’s input on the mobile device. 
@app.route('/api/game/user)
Input JSON format example: {"userid" : "user01", "firstname" : "Arturo"} 

Player State Synchronization: Based on UserID, provide user data through a response to the client from the database.
@app.route('/api/game/user/<userid>')
Input : UserID
Response JSON format : {"userid" : "user01", "firstname" : "Arturo"} 

Player location: Instantaneously collect the player’s location data (latitude and longitude coordinates), and update this information in the database when the player moves according to the set thresholds (one update every XX)
@app.route('/api/game/location‘)
Input JSON format example: {"latitude":  "32.7877", "longitude" : " -96.8062"}

List of Objects around the player’s location: Send a response to the client reporting all objects avaialbe for collection, based on current position within a specified radius.
@app.route('/api/game/getlist/<userid>/<r>)
Input : UserID and Radius
Reponse JSON format : List of all available objects

Colission report: Send a response to the client reporting the game score and revealing new story segments, which are triggered according to the SegmentThreshold parameter in the database. 
