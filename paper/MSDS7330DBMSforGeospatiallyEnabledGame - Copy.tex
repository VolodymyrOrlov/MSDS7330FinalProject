...
As mentioned, the game consists of players exploring the real world via augmented reality in search of virtual objects. Collecting objects raises the players score which will in turn reveal a segment of a story once the players score reaches a certain threshold. The virtual objects belong to different categories that correspond to different scores that must be earned and different aspects of the story that may be revealed. As such, this game relies heavily on the search-and-find mechanic as well as desires for collection, completion, and closure of narrative for player engagement \cite{game-methodology, location-based-games}. 
...
\textit{Details:}
Example of input in JSON format: \big\{"userid" : "user01", "firstname" : "Arturo"\big\}


\textbf{Player State Synchronization:} Based on UserID, provide user data through a response to the client from the database.

\textit{Details:}
Input : UserID, Response in JSON format : \big\{"userid" : "user01", "firstname" : "Arturo"\big\}
...
\textit{Details:}
Example of input in JSON format: \big\{"latitude":  "32.7877", "longitude" : " -96.8062"\big\}
...
\textit{Details:}
Input : UserID and Radius, Response in JSON format : List of all available objects
...
\textit{Details:}
Response in JSON format : Current Score and Story Segment and story based on score

\textbf{User score status per Game category:} 

\textit{Details:}
Input : UserID, Response in JSON format : ScoreCategory and Current score
